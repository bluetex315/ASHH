package Hyper_Heuristics.xcj.shafi;
	...
	public void solve(ProblemDomain problem) {
		...
		while (!hasTimeExpired())
		{
			...
			for (i=0; i<number_explore_heuristics; i++)
			{
				...
				{
					System.out.println("XCJ 94 heuristic "+heuristic_to_apply);
					System.out.println("XCJ 95 prev obj "+problem.getFunctionValue(0));
					System.out.println("XCJ 96 prev obj "+problem.getFunctionValue(heuristic_to_apply+1));
					curr_obj_value = problem.applyHeuristic(heuristic_to_apply, 0, heuristic_to_apply+1, heuristic_to_apply+1);	
					System.out.println("XCJ 96 curr obj "+problem.getFunctionValue(heuristic_to_apply+1));
				}
				else
				{
					System.out.println("XCJ 102 heuristic "+heuristic_to_apply);
					System.out.println("XCJ 103 prev obj "+problem.getFunctionValue(0));
					System.out.println("XCJ 104 prev obj "+problem.getFunctionValue(heuristic_to_apply+1));
					curr_obj_value = problem.applyHeuristic(heuristic_to_apply, 0, heuristic_to_apply+1);
					System.out.println("XCJ 106 prev obj "+problem.getFunctionValue(heuristic_to_apply+1));
				}
			}			
			...
			for (i=0; i<number_explore_heuristics; i++)
			{
				for (j=0; j<number_exploit_heuristics; j++)
				{
					...
					{
						System.out.println("XCJ 119 heuristic "+heuristic_to_apply);
						System.out.println("XCJ 120 prev obj "+problem.getFunctionValue(explore_heuristic_indices.get(i)+1));
						curr_obj_value = problem.applyHeuristic(heuristic_to_apply, explore_heuristic_indices.get(i)+1, heuristic_to_apply+1);
						System.out.println("XCJ 122 prev obj "+problem.getFunctionValue(heuristic_to_apply+1));
						double delta = 0.0;
						if (curr_obj_value < best_obj_value)
						{
							best_obj_value = curr_obj_value;
