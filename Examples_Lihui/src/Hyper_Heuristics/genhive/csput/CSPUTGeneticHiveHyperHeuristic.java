package Hyper_Heuristics.genhive.csput;
...
public class CSPUTGeneticHiveHyperHeuristic extends HyperHeuristic {
	...
	private void evaluateAgents(Vector<HiveAgent> agents, ProblemDomain problem) {
		...
		for (int i = 0; i < agents.size() && !hasTimeExpired(); i++) {
			...
			System.out.println("GenHive 46 hh: "+code.get(0));
            System.out.println("GenHive 47 prev q "+problem.getFunctionValue(source));
			problem.applyHeuristic(code.get(0), source, new_location);
			System.out.println("GenHive 50 curr q "+problem.getFunctionValue(new_location));

			for (int j = 1; j < code.size(); j++) {
				System.out.println("GenHive 52 hh: "+code.get(j));
	            System.out.println("GenHive 53 prev q "+problem.getFunctionValue(new_location));
				problem.applyHeuristic(code.get(j), new_location, new_location);
				System.out.println("GenHive 55 curr q "+problem.getFunctionValue(new_location));
			}
