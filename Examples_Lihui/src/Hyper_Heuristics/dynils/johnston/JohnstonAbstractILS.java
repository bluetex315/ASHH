package Hyper_Heuristics.dynils.johnston;
...
public abstract class JohnstonAbstractILS extends HyperHeuristic {
	...
	protected void doLocalSearch(int index) {
		for (int i = 0; i < localSearchHeuristics.length; i++) {
			System.out.println("DynILS 55 hh: "+localSearchHeuristics[i]);
            System.out.println("DynILS 56 prev q "+problem.getFunctionValue(index));
			problem.applyHeuristic(localSearchHeuristics[i], index, index);
			System.out.println("DynILS 57 prev q "+problem.getFunctionValue(index));
		}
	}
	...
	protected void kickAndClimb(int heuristicToApply, int perturbationStrength) {
		...
		System.out.println("DynILS 125 hh: "+heuristicToApply);
        System.out.println("DynILS 126 prev q "+problem.getFunctionValue(0));
		problem.applyHeuristic(heuristicToApply, 0, 1);
		System.out.println("DynILS 128 prev q "+problem.getFunctionValue(1));
