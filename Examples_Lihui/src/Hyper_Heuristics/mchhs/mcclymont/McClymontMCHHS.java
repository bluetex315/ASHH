package Hyper_Heuristics.mchhs.mcclymont;
...
this.numberOfSolutions);
        ...
        while (!hasTimeExpired()) {
            ...
            if (crossover[h]) {
                ...
                System.out.println("MCHH-S 81 heuristic "+h);
				System.out.println("MCHH-S 82 prev obj "+problem.getFunctionValue(i+a));
				System.out.println("MCHH-S 83 prev obj "+problem.getFunctionValue(c));
                objectives[i + b] = problem.applyHeuristic(h, i + a, c, i + b);
                System.out.println("MCHH-S 85 curr obj "+problem.getFunctionValue(i+b));
            } else {
            	System.out.println("MCHH-S 87 heuristic "+h);
				System.out.println("MCHH-S 88 prev obj "+problem.getFunctionValue(i+a));
                objectives[i + b] = problem.applyHeuristic(h, i + a, i + b);
                System.out.println("MCHH-S 89 curr obj "+problem.getFunctionValue(i+b));
            }
            if(!hasTimeExpired())
            {
                    ...
                    if (objectives[i] > objectives[i + this.numberOfSolutions] || rng.nextBoolean()) 
{
                        ...
                        if (localSearchIndicies.length > 0 && rng.nextDouble() > 0.35) {
                            ...
                            System.out.println("MCHH-S 105 heuristic "+h);
            				System.out.println("MCHH-S 106 prev obj "+problem.getFunctionValue(i+b));
                            problem.applyHeuristic(h, i + b, i + b);
                            System.out.println("MCHH-S 106 curr obj "+problem.getFunctionValue(i+b));
...
0.35) {
                        ...
                        if (localSearchIndicies.length > 0 && rng.nextBoolean()) {
                            time = System.nanoTime();
                            h = localSearchIndicies[rng.nextInt(localSearchIndicies.length)];
                            System.out.println("MCHH-S 139 heuristic "+h);
            				System.out.println("MCHH-S 140 prev obj "+problem.getFunctionValue(i+b));
                            problem.applyHeuristic(h, i + b, i + b);
                            System.out.println("MCHH-S 140 curr obj "+problem.getFunctionValue(i+b));
                            flag = primaryActive[i];
                            primaryActive[i] = objectives[i] >
                                    objectives[i + this.numberOfSolutions];
                            if(flag != primaryActive[i]){
