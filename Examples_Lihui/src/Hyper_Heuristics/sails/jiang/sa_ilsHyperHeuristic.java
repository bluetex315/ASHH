package Hyper_Heuristics.sails.jiang;
...
public class sa_ilsHyperHeuristic extends HyperHeuristic {
	...
	public void solve(ProblemDomain problem) {
            ...
            if(local_search!=null){
                ...
                for(int i=0;i<local_search.length;i++){
                	System.out.println("SA-ILS 236 heuristic "+local_search[i]);
                	System.out.println("SA-ILS 237 prev obj "+problem.getFunctionValue(0));
                    best_value=problem.applyHeuristic(local_search[i], 0, 0);
                    System.out.println("SA-ILS 238 prev obj "+problem.getFunctionValue(0));
                    if(i%2==0)
                        hasTimeExpired();
                }
                ...
                while(!hasTimeExpired()){
                    ...
                    if(problem_consumed){
                        ...
                        if(selection<=m){
                            ...
                            System.out.println("SA-ILS 110 heuristic "+heuristic_to_apply);
            				System.out.println("SA-ILS 111 prev obj "+problem.getFunctionValue(0));
                            problem.applyHeuristic(heuristic_to_apply,0,1);
            				System.out.println("SA-ILS 113 curr obj "+problem.getFunctionValue(1));
                        }
                        else if(selection>m&&selection<=(m+rr)){
                            heuristic_to_apply = ruin_recreate[rng.nextInt(rr)];
                            System.out.println("SA-ILS 117 heuristic "+heuristic_to_apply);
            				System.out.println("SA-ILS 118 prev obj "+problem.getFunctionValue(0));
                            problem.applyHeuristic(heuristic_to_apply,0,1);
            				System.out.println("SA-ILS 120 curr obj "+problem.getFunctionValue(1));
                        }
                        else{
                            heuristic_to_apply = crossover[rng.nextInt(c)];
                            System.out.println("SA-ILS 160 heuristic "+heuristic_to_apply);
                            System.out.println("SA-ILS 161 prev obj "+problem.getFunctionValue(0));
            				System.out.println("SA-ILS 161 prev obj "+problem.getFunctionValue(1));
                        ...
                        while(times<times_set){
                            ...
                            System.out.println("SA-ILS 163 prev obj "+problem.getFunctionValue(1));
                        ...
                        if(new_value<best_value){
                            ...
                            if(selection<=m){
                                heuristic_to_apply = mutation[rng.nextInt(m)];
                                System.out.println("SA-ILS 182 heuristic "+heuristic_to_apply);
                				System.out.println("SA-ILS 183 prev obj "+problem.getFunctionValue(0));
                                best_value=problem.applyHeuristic(heuristic_to_apply,0,1);
                                System.out.println("SA-ILS 185 curr obj "+problem.getFunctionValue(1));
                            }
                            else if(selection>m&&selection<=(m+rr)){
                                heuristic_to_apply = ruin_recreate[rng.nextInt(rr)];
                                System.out.println("SA-ILS 189 heuristic "+heuristic_to_apply);
                				System.out.println("SA-ILS 190 prev obj "+problem.getFunctionValue(0));
                                best_value=problem.applyHeuristic(heuristic_to_apply,0,1);
                                System.out.println("SA-ILS 192 curr obj "+problem.getFunctionValue(1));
                            }
                            else{
                                heuristic_to_apply = crossover[rng.nextInt(c)];
                                System.out.println("SA-ILS 196 heuristic "+heuristic_to_apply);
                				System.out.println("SA-ILS 197 prev obj "+problem.getFunctionValue(0));
                				System.out.println("SA-ILS 198 prev obj "+problem.getFunctionValue(1));
                                best_value=problem.applyHeuristic(heuristic_to_apply,0,1,1);
                                System.out.println("SA-ILS 200 curr obj "+problem.getFunctionValue(1));
                    ...
                    for(int i=0;i<number_of_heuristics;i++){
                        ...
                        for(int j=0;j<c;j++){
                            if(i==crossover[j]){
                                h_is_crossover=true;
                                if(tabu_table[i]==0)
                                	System.out.println("SA-ILS 236 heuristic "+i);
                                	System.out.println("SA-ILS 237 prev obj "+problem.getFunctionValue(number_of_heuristics));
                                	System.out.println("SA-ILS 238 prev obj "+problem.getFunctionValue(number_of_heuristics+1));
                                    values[i]=problem.applyHeuristic(i, number_of_heuristics,number_of_heuristics+1, i);
                                    System.out.println("SA-ILS 238 curr obj "+problem.getFunctionValue(i));
                                break;
                            }
                        }
                        if(!h_is_crossover&&tabu_table[i]==0)
                        	System.out.println("SA-ILS 160 heuristic "+i);
	        				System.out.println("SA-ILS 161 prev obj "+problem.getFunctionValue(number_of_heuristics));
                            values[i]=problem.applyHeuristic(i, number_of_heuristics, i);
                            System.out.println("SA-ILS 163 prev obj "+problem.getFunctionValue(i));
                    }
