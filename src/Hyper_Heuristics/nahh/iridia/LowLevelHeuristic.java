...
//            throw new UnsupportedOperationException("Parameter setting not supported for this low level heuristic.");
    ...
    private double doExecute(int src, int dst) {
        if (type == ProblemDomain.HeuristicType.LOCAL_SEARCH) {
            ...
            do {
                b_qual = qual;
                System.out.println("NAHH 119 heuristic "+id);
				System.out.println("NAHH 120 prev obj "+Shared.problem.getFunctionValue(posTempSolution));
                qual = Shared.problem.applyHeuristic(id, posTempSolution, dst);
                System.out.println("NAHH 120 prev obj "+Shared.problem.getFunctionValue(dst));
                Shared.problem.copySolution(dst, posTempSolution);
            } while (!Shared.hyperHeuristic.hasTimeExpired() && b_qual - qual > confPrecision);
            return qual;
        } else {
        	double qual_;
        	System.out.println("NAHH 127 heuristic "+id);
			System.out.println("NAHH 128 prev obj "+Shared.problem.getFunctionValue(src));
			qual_ = Shared.problem.applyHeuristic(id, src, dst);
			System.out.println("NAHH 128 prev obj "+Shared.problem.getFunctionValue(dst));
			return qual_;
        }
    }
