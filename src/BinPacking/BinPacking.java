...
public class BinPacking extends ProblemDomain {
	...
	public static String[] getFileName() {
        String path = System.getProperty("user.dir") + "/instances/binpacking/"; //path
        System.out.println("binpacking line59 "+path);
...
//            		System.out.println("Binpacking line83:"+fa[i].getParent());
            		int nameIndex = fa[i].getParent().indexOf("binpacking");
            		String rootName = fa[i].getParent().substring(nameIndex+"binpacking".length()+1);
...
//            		System.out.println(resultName+" "+"is put into stack");
	...
	private void readInInstance(BufferedReader buffread) {
		try {
//			System.out.println("BP line176 "+totalpiecesize);
			buffread.readLine();
...
//	}
	...
	public void loadInstance(int instanceID) {
		...
		if (instanceID < instanceNames.length) {
//			ins = "instances/binpacking/falk1000-" + (instanceID+1) + ".txt";
			ins = "instances/binpacking/" + instanceNames[instanceID];
		}
		else {
			System.err.println("instance " + ins + "does not exist");
			System.exit(-1);
		}
		System.out.println("the current instance is: " + ins);
		loadInstance(ins); //load the instance from the file
		solutionMemory = new Solution[defaultmemorysize];
		
		//set solution memory size
		initialiseSolution(0);//load all of the pieces into the bins to initialise the solution
	}
...
//		System.out.println("BP line852 "+v.size());
		for (int x = 0; x < v.size(); x++) {//loop bins
			Bin b = v.get(x);
			totalnumberofpieces += b.numberOfPiecesInThisBin();
//			System.out.println("BP line855 "+b.getFullness());
		...
		if (totalfullness != totalpiecesize) {
			...
			System.out.println(totalpiecesize);
